...
| `MSMODELSLIM_ALLOW_TRUNCATED_CHECKPOINT` | `string` | 可选 | 未设置 | `1`、`true`、`yes`、`on` | 允许量化不完整（被截断）的权重目录。两种不完整都会被识别：`model.safetensors.index.json` 引用的分片有缺失，或 index 本身就没列全某一层需要的权重（逐层在 meta device 上搭出该层取 `named_parameters` 精确比对，不依赖命名规律，因此 `attn.indexer`、`ffn.gate.tid2eid` 这类逐层不同的结构不会被误判）。设置后把层数收敛到从第 0 层起连续完整的那些层，只量化这些层。**产物仅用于打通流程，精度无意义**；未设置时会在加载权重前直接报错，并指明缺的是哪些分片、哪一层缺哪个权重。 |
